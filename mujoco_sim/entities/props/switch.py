...
class Switch(composer.Entity):
    ...
    def _build(self, target_force_range=(5, 200)):
        ...
        self._cube_geom = self._mjcf_model.worldbody.add(
            'geom', type='box', size=[self.box_size/2, self.box_size/2, self.box_height/2], rgba=[1, 1, 1, 1], pos=[0, 0, self.box_height/2])
        self._button_geom = self._mjcf_model.worldbody.add(
            'geom', type='cylinder', size=[self.box_size/2.5,self.box_size/2.5,0.01], rgba=[1, 0, 0, 1], pos=[0, 0, self.box_height])
    ...
    def _update_activation(self, physics):
        ...
        self._is_pressed = (current_force >= self._min_force and
                            current_force <= self._max_force)
        
        # flip state on rising edge
        if self._is_pressed and not self.was_pressed:
            self._is_active = not self._is_active
        physics.bind(self._button_geom).rgba = (
            [0, 1, 0, 1] if self._is_active else [1, 0, 0, 1])
