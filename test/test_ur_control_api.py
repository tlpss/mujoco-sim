...
def test_moveJ():
    ...
    assert np.allclose(robot.get_joint_positions(physics), joints, atol=1e-2), f"Joint mismatch: {robot.get_joint_positions(physics)} vs {joints}"
...
def test_moveJ_IK():
        ...
        assert np.allclose(robot.get_tcp_pose(physics), target_pose, atol=1e-2), f"Pose mismatch: {robot.get_tcp_pose(physics)} vs {target_pose}"
